...
//  const _translate = JSON.parse(fs.readFileSync(`./language/${idioma}.json`));
 // const tradutor = _translate.plugins.descargas_play;
  ...
  if (!text) throw `Debes escribir algo después del comando _${usedPrefix + command}_`;
  ...
  const texto1 = `Título: ${yt_play[0].title}\nPublicado: ${yt_play[0].ago}\nDuración: ${secondString(yt_play[0].duration.seconds)}\nVistas: ${MilesNumber(yt_play[0].views)}\nAutor: ${yt_play[0].author.name}\nVideo ID: ${yt_play[0].videoId}\nTipo: ${yt_play[0].type}\nURL: ${yt_play[0].url}\nAutor URL: ${yt_play[0].author.url}\n\n> Descargando ${additionalText}. Por favor espera...`.trim();
  ...
  if (command === 'play') {
    try {
      ...
      if (size >= limit_a2) {
        await conn.sendMessage(m.chat, { text: `El archivo es muy grande para enviarlo directamente. Aquí tienes el enlace: _${resultados.descargar}_` }, { quoted: m });
        return;
      }
      ...
      try {
        ...
        if (size >= limit_a2) {
          await conn.sendMessage(m.chat, { text: `El archivo es muy grande para enviarlo directamente. Aquí tienes el enlace: _${audio}_` }, { quoted: m });
          return;
        }
        if (size >= limit_a1 && size <= limit_a2) {
          await conn.sendMessage(m.chat, { document: buff_aud, mimetype: 'audio/mpeg', fileName: ttl + `.mp3` }, { quoted: m });
        ...
        throw 'Hubo un error al intentar descargar el audio.';
      }
    }
  }
  ...
  if (command === 'play2') {
    try {
      ...
      try {
        ...
        if (size2 >= limit2) {
          await conn.sendMessage(m.chat, { text: `El archivo es muy grande para enviarlo directamente. Aquí tienes el enlace: _${video}_` }, { quoted: m });
          return;
        }
        if (size2 >= limit1 && size2 <= limit2) {
          await conn.sendMessage(m.chat, { document: buff_vid, mimetype: 'video/mp4', fileName: ttl2 + `.mp4` }, { quoted: m });
        ...
        throw 'Hubo un error al intentar descargar el video.';
      }
    }
  }
};
