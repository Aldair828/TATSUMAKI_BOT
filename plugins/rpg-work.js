...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    // Establecer un rango de créditos a ganar, entre 5 y 20 créditos
    let minCréditos = 5
    let maxCréditos = 20
    let amount = Math.floor(Math.random() * (maxCréditos - minCréditos + 1)) + minCréditos

    // Añadir créditos al usuario
    user.limit += amount
    await m.reply(`${works.getRandom()} *${amount}* créditos.`)
